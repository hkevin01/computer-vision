...
namespace stereo_vision
{
    namespace gpu
    {
        ...
        void GPUStereoMatcher::deallocateMemory()
        {
            ...
            if (d_left_image_)
            {
                GPU_CHECK(gpuFree(d_left_image_));
                d_left_image_ = nullptr;
            }
            if (d_right_image_)
            {
                GPU_CHECK(gpuFree(d_right_image_));
                d_right_image_ = nullptr;
            }
            if (d_disparity_)
            {
                GPU_CHECK(gpuFree(d_disparity_));
                                                   ...
                                                   int min_disp, int max_disp, int block_size)
        {
            ...
            // Launch HIP kernel using hipLaunchKernel
            {
                void *kernelArgs[] = {
                    &d_left_image_, &d_right_image_, &d_disparity_,
                    &width_, &height_, &min_disp, &max_disp, &block_size};
                GPU_CHECK(hipLaunchKernel(
                    (const void *)stereo_matching_kernel,
                    grid_dim, block_dim,
                    kernelArgs,
                    0,   // shared memory
                    0)); // default stream
