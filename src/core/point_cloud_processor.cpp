...
namespace stereo_vision
{
        ...
        const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud)
    {
        ...
        for (const auto &point : cloud->points)
        {
            ...
            if (point.x < stats.min_point.x)
                stats.min_point.x = point.x;
            if (point.y < stats.min_point.y)
                stats.min_point.y = point.y;
            if (point.z < stats.min_point.z)
                stats.min_point.z = point.z;

            if (point.x > stats.max_point.x)
                stats.max_point.x = point.x;
            if (point.y > stats.max_point.y)
                stats.max_point.y = point.y;
            if (point.z > stats.max_point.z)
                stats.max_point.z = point.z;
        }
