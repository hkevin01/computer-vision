...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), m_centralWidget(nullptr), m_mainSplitter(nullptr),
      ...
      m_processingTimer(new QTimer(this)), m_isProcessing(false),
      m_hasCalibration(false), m_hasImages(false) {
...
void MainWindow::loadCalibration() {
    ...
    if (m_calibration->loadCalibration(fileName.toStdString(), m_stereo_params)) {
        QMessageBox::information(this, tr("Success"), tr("Calibration loaded successfully."));
        // Update UI elements if necessary
        if (m_parameterPanel) {
            m_parameterPanel->setStereoParameters(m_stereo_params);
        }
    } else {
        QMessageBox::critical(this, tr("Error"), tr("Failed to load calibration file."));
    }
}

void MainWindow::saveCalibration() {
    QString fileName = QFileDialog::getSaveFileName(this,
        tr("Save Calibration File"), "",
        tr("YAML Files (*.yml *.yaml);;All Files (*)"));
    ...
    if (m_calibration->saveCalibration(fileName.toStdString(), m_stereo_params)) {
        QMessageBox::information(this, tr("Success"), tr("Calibration saved successfully."));
    } else {
        QMessageBox::critical(this, tr("Error"), tr("Failed to save calibration file."));
    }
}
